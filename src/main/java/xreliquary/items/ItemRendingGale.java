...
public class ItemRendingGale extends ItemToggleable {
    ...
    public void addInformation(ItemStack ist, EntityPlayer player, List list, boolean par4) {
        ...
        this.formatTooltip(
            ImmutableMap.of("charge", Integer.toString(NBTHelper.getInteger("feathers", ist))),
            ist,
            list);
        ...
        if (this.isEnabled(ist)) LanguageHelper.formatTooltip(
            ...
            ImmutableMap.of(
                "item",
                EnumChatFormatting.WHITE + Items.feather.getItemStackDisplayName(new ItemStack(Items.feather))),
            ist,
            list);
    ...
    private static int getChargeLimit() {
        return Reliquary.CONFIG.getInt(Names.rending_gale, "charge_limit");
    }

    public static int getChargeCost() {
        return Reliquary.CONFIG.getInt(Names.rending_gale, "cast_charge_cost");
    }

    private static int getFeathersWorth() {
        return Reliquary.CONFIG.getInt(Names.rending_gale, "charge_feather_worth");
    }

    private static int getBoltChargeCost() {
        return Reliquary.CONFIG.getInt(Names.rending_gale, "bolt_charge_cost");
    }

    private static int getBoltTargetRange() {
        return Reliquary.CONFIG.getInt(Names.rending_gale, "block_target_range");
    }

    private static int getRadialPushRadius() {
        return Reliquary.CONFIG.getInt(Names.rending_gale, "push_pull_radius");
    }

    private static boolean canPushProjectiles() {
        return Reliquary.CONFIG.getBool(Names.rending_gale, "can_push_projectiles");
    }
            ...
            if (Math.abs(x) > Math.abs(y) && Math.abs(x) > Math.abs(z)) {
                if (!isAABBInAnythingButAir(player.worldObj, player.boundingBox.getOffsetBoundingBox(x, 0D, 0D))) {
                    ...
                    if (Math.abs(z) > Math.abs(y)) {
                        if (!isAABBInAnythingButAir(
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(0D, 0D, z))) {
                            // z is fine
                            y = 0D;
                        } else if (!isAABBInAnythingButAir(
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(0D, y, 0D))) z = 0D;
                    } else {
                        if (!isAABBInAnythingButAir(
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(0D, y, 0D))) {
                            // y is fine
                            z = 0D;
                        } else if (!isAABBInAnythingButAir(
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(0D, 0D, z))) y = 0D;
                    }
                } else {
                    // x is not fine
                    x = 0D;
                ...
                if (!isAABBInAnythingButAir(player.worldObj, player.boundingBox.getOffsetBoundingBox(0D, 0D, z))) {
                    ...
                    if (Math.abs(x) > Math.abs(y)) {
                        if (!isAABBInAnythingButAir(
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(x, 0D, 0D))) {
                            // x is fine
                            y = 0D;
                        } else if (!isAABBInAnythingButAir(
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(0D, y, 0D))) x = 0D;
                    } else {
                        if (!isAABBInAnythingButAir(
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(0D, y, 0D))) {
                            ...
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(x, 0D, 0D))) y = 0D;
                    }
                } else {
                    // z is not fine
                    z = 0D;
                ...
                if (!isAABBInAnythingButAir(player.worldObj, player.boundingBox.getOffsetBoundingBox(0D, y, 0D))) {
                    ...
                    if (Math.abs(x) > Math.abs(z)) {
                        if (!isAABBInAnythingButAir(
                            ...
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(0D, 0D, z))) x = 0D;
                    } else {
                        if (!isAABBInAnythingButAir(
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(x, 0D, 0D))) {
                    ...
                    if (Math.abs(x) > Math.abs(z)) {
                        ...
                        if (!isAABBInAnythingButAir(
                            ...
                            player.worldObj,
                            player.boundingBox.getOffsetBoundingBox(x, 0D, 0D))) z = 0D;
                    }
                }
            }
            if (isAABBInAnythingButAir(player.worldObj, player.boundingBox.getOffsetBoundingBox(x, y, z))) {
    ...
    public void cycleMode(ItemStack ist, boolean isRaining) {
        if (getMode(ist).equals("flight")) setMode(ist, "push");
        else if (getMode(ist).equals("push")) setMode(ist, "pull");
        else if (getMode(ist).equals("pull") && isRaining) setMode(ist, "bolt");
        else setMode(ist, "flight");
    }
    ...
    public void onUsingTick(ItemStack ist, EntityPlayer player, int count) {
        if (NBTHelper.getInteger("feathers", ist) < getChargeCost()) return;
        count -= 1;
        count = getMaxItemUseDuration(ist) - count;
        if (count == getMaxItemUseDuration(ist)
            || (getMaxItemUseDuration(ist) - count) * getChargeCost() >= NBTHelper.getInteger("feathers", ist)) {
        ...
        if (getMode(ist).equals("flight")) {
            ...
            // doPushEffect(player, player.worldObj, player.posX, player.posY + player.getEyeHeight(), player.posZ,
            // player.getLookVec());
            // spawnFlightParticles(player.worldObj, player.posX, player.posY + player.getEyeHeight(), player.posZ,
            // player.getLookVec());
                    ...
                    if (player.worldObj.canLightningStrikeAt(mop.blockX, attemptedY, mop.blockZ)) {
                        if (NBTHelper.getInteger("feathers", ist) >= getBoltChargeCost()) {
                            NBTHelper.setInteger(
                                "feathers",
                                ist,
                                NBTHelper.getInteger("feathers", ist) - getBoltChargeCost());
    ...
    public void onPlayerStoppedUsing(ItemStack ist, World world, EntityPlayer player, int count) {
        ...
        NBTHelper.setInteger(
            "feathers",
            ist,
            NBTHelper.getInteger("feathers", ist) - Math.min(chargeUsed, NBTHelper.getInteger("feathers", ist)));
    }

    public void doRadialPush(EntityPlayer player, boolean pull) {
        // push effect free at the moment, if you restore cost, remember to change this to
        // NBTHelper.getInteger("feathers", ist)
        ...
        List<String> entitiesThatCanBePushed = (List<String>) Reliquary.CONFIG
            .get(Names.rending_gale, "entities_that_can_be_pushed");
        List<String> projectilesThatCanBePushed = (List<String>) Reliquary.CONFIG
            .get(Names.rending_gale, "projectiles_that_can_be_pushed");

        List eList = player.worldObj.getEntitiesWithinAABB(
            Entity.class,
            AxisAlignedBB.getBoundingBox(lowerX, lowerY, lowerZ, upperX, upperY, upperZ));
        ...
        while (iterator.hasNext()) {
            ...
            if (entitiesThatCanBePushed.contains(entityName)
                || (!pull && canPushProjectiles() && projectilesThatCanBePushed.contains(entityName))) {
                double distance = player.getDistanceToEntity(e);
                if (distance >= getRadialPushRadius()) continue;

                if (e.equals(player)) continue;
                Vec3 pushVector;
                if (pull) {
                    pushVector = Vec3
                        .createVectorHelper(player.posX - e.posX, player.posY - e.posY, player.posZ - e.posZ);
                } else {
                    pushVector = Vec3
                        .createVectorHelper(e.posX - player.posX, e.posY - player.posY, e.posZ - player.posZ);
                }
    ...
    public void spawnFlightParticles(World world, double x, double y, double z, Vec3 lookVector) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            world.spawnParticle(
                "blockdust_" + Block.getIdFromBlock(Blocks.snow_layer) + "_" + 0,
                x + randX,
                y + randY,
                z + randZ,
                lookVector.xCoord * 5,
    ...
    public void spawnRadialHurricaneParticles(EntityPlayer player, boolean pull) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            player.worldObj.spawnParticle(
                "blockdust_" + Block.getIdFromBlock(Blocks.snow_layer) + "_" + 0,
                player.posX + randX,
                (player.posY + player.getEyeHeight()) - (player.height / 2),
                player.posZ + randZ,
                motX,
                0.0D,
                motZ);
