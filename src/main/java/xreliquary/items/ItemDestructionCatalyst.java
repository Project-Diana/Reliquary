...
public class ItemDestructionCatalyst extends ItemToggleable {
    ...
    public static List<String> ids = ImmutableList.of(
        "minecraft:dirt",
        "minecraft:grass",
        "minecraft:gravel",
    ...
    public void addInformation(ItemStack ist, EntityPlayer player, List list, boolean par4) {
        ...
        this.formatTooltip(
            ImmutableMap.of("charge", Integer.toString(NBTHelper.getInteger("gunpowder", ist))),
            ist,
            list);
        if (this.isEnabled(ist)) LanguageHelper.formatTooltip(
            ...
            ImmutableMap.of(
                "item",
                EnumChatFormatting.GRAY + Items.gunpowder.getItemStackDisplayName(new ItemStack(Items.gunpowder))),
            ist,
            list);
            ...
            for (int yD = -getExplosionRadius(); yD <= getExplosionRadius(); yD++) {
                for (int zD = -getExplosionRadius(); zD <= getExplosionRadius(); zD++) {
                    if (!perfectCube()) {
                        ...
                        if (distance >= getExplosionRadius()) continue;
                    }
                    ...
                    if (isBreakable(ContentHelper.getIdent(world.getBlock(x + xD, y + yD, z + zD)))) {
                        ...
                        if (world.rand.nextInt(2) == 0) {
                            world.spawnParticle(
                                "largeexplode",
                                x + xD + (world.rand.nextFloat() - 0.5F),
                                y + yD + (world.rand.nextFloat() - 0.5F),
                                z + zD + (world.rand.nextFloat() - 0.5F),
                                0.0D,
                                0.0D,
                                0.0D);
                        }
    ...
    private int gunpowderWorth() {
        return Reliquary.CONFIG.getInt(Names.destruction_catalyst, "gunpowder_worth");
    }

    private int gunpowderLimit() {
        return Reliquary.CONFIG.getInt(Names.destruction_catalyst, "gunpowder_limit");
    }
}
