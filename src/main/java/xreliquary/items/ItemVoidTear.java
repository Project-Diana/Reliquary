...
public class ItemVoidTear extends ItemToggleable {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        ...
        if (this.isEnabled(stack)) {
            LanguageHelper.formatTooltip(
                ...
                ImmutableMap.of("item", EnumChatFormatting.YELLOW + contents.getDisplayName()),
                stack,
                list);
            list.add(LanguageHelper.getLocalization("tooltip.absorb_tear"));
        }
        LanguageHelper.formatTooltip(
            "tooltip.tear_quantity",
            ImmutableMap.of("item", contents.getDisplayName(), "amount", Integer.toString(contents.stackSize)),
            stack,
            list);
    }
    ...
    public ItemStack onItemRightClick(ItemStack ist, World world, EntityPlayer player) {
        if (!world.isRemote) {
            ...
            if (this
                .attemptToEmptyIntoInventory(ist, player, player.inventory, player.inventory.mainInventory.length)) {
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int i, boolean f) {
        if (!world.isRemote) {
            if (!this.isEnabled(stack) || !(entity instanceof EntityPlayer)) return;

            EntityPlayer player = (EntityPlayer) entity;

            ItemStack contents = this.getContainedItem(stack);
            if (contents.stackSize < Reliquary.CONFIG.getInt(Names.void_tear, "item_limit")
                && InventoryHelper.consumeItem(contents, player, contents.getMaxStackSize())) {
                // doesn't absorb in creative mode.. this is mostly for testing, it prevents the item from having
                // unlimited *whatever* for eternity.
                if (!player.capabilities.isCreativeMode) this.onAbsorb(stack, player);
            }
    ...
    public void attemptToReplenishSingleStack(EntityPlayer player, ItemStack ist) {
        ...
        if (stackCount == 1 && preferredSlot != -1) {
            ...
            if (stackToIncrease == null) return;
            while (stackToIncrease.stackSize < stackToIncrease.getMaxStackSize()
                && getContainedItem(ist).stackSize > 1) {
                ...
                if (this.isEnabled(ist)) {
                    ...
                    if (this.attemptToEmptyIntoInventory(ist, player, inventory, 0)) {
                        ...
                        player.inventory.setInventorySlotContents(
                            ...
                            new ItemStack(Reliquary.CONTENT.getItem(Names.void_tear_empty), 1, 0));
                    }
                }
                return true;
            }
    ...
    public ItemStack getContainedItem(ItemStack ist) {
        // something awful happened. We either lost data or this is an invalid tear by some other means. Either way, not
        // great.
        if (NBTHelper.getString("itemID", ist)
            .equals("")) return null;
        return new ItemStack(
            (Item) Item.itemRegistry.getObject(NBTHelper.getString("itemID", ist)),
            NBTHelper.getInteger("itemQuantity", ist),
            NBTHelper.getShort("itemMeta", ist));
    }
    ...
    protected void drainInventory(ItemStack ist, EntityPlayer player, IInventory inventory) {
        ...
        if (foundItem) player.worldObj.playSoundAtEntity(
            player,
            "random.orb",
            0.1F,
            0.5F * ((player.worldObj.rand.nextFloat() - player.worldObj.rand.nextFloat()) * 0.7F + 1.2F));
    ...
    public boolean tryToAddToInventory(ItemStack contents, IInventory inventory, int limit) {
        for (int slot = 0; slot
            ...
            if (inventory.getStackInSlot(slot)
                .isItemEqual(contents)) {
                if (inventory.getStackInSlot(slot).stackSize == inventory.getStackInSlot(slot)
                    .getMaxStackSize()) {
                    continue;
                }
                inventory.setInventorySlotContents(
                    ...
                    new ItemStack(
                        contents.getItem(),
                        inventory.getStackInSlot(slot).stackSize + 1,
        ...
        for (int slot = 0; slot
            ...
            if (inventory.getStackInSlot(slot) == null) {
                inventory.setInventorySlotContents(
                    slot,
                    new ItemStack(contents.getItem(), contents.stackSize, contents.getItemDamage()));
                return true;
            }
        }
        return false;
