...
public class ItemHarvestRod extends ItemToggleable {
    ...
    public void addInformation(ItemStack ist, EntityPlayer player, List list, boolean par4) {
        ...
        this.formatTooltip(
            ImmutableMap.of("charge", Integer.toString(NBTHelper.getInteger("bonemeal", ist))),
            ist,
            list);
        if (this.isEnabled(ist)) LanguageHelper.formatTooltip(
            ...
            ImmutableMap.of(
                ...
                EnumChatFormatting.WHITE
                    + Items.dye.getItemStackDisplayName(new ItemStack(Items.dye, 1, Reference.WHITE_DYE_META))),
            ist,
            list);
    ...
    public int getBonemealLimit() {
        return Reliquary.CONFIG.getInt(Names.harvest_rod, "bonemeal_limit");
    }

    public int getBonemealWorth() {
        return Reliquary.CONFIG.getInt(Names.harvest_rod, "bonemeal_worth");
    }

    public int getBonemealCost() {
        return Reliquary.CONFIG.getInt(Names.harvest_rod, "bonemeal_cost");
    }

    public int getLuckRolls() {
        return Reliquary.CONFIG.getInt(Names.harvest_rod, "bonemeal_luck_rolls");
    }

    public int getLuckPercent() {
        return Reliquary.CONFIG.getInt(Names.harvest_rod, "bonemeal_luck_percent_chance");
    }

    public int getBreakRadius() {
        return Reliquary.CONFIG.getInt(Names.harvest_rod, "harvest_break_radius");
    }
    ...
    public void doHarvestBlockBreak(ItemStack ist, int x, int y, int z, EntityPlayer player, int xOff, int yOff,
        int zOff) {
        x += xOff;
        y += yOff;
        z += zOff;
        ...
        if (!(block instanceof IPlantable) && !(block instanceof BlockCrops)) return;
        if (block instanceof BlockFertileLilypad) return;
        ...
        if (player.worldObj.isRemote) {
            for (int particles = 0; particles <= 8; particles++) player.worldObj.playAuxSFXAtEntity(
                player,
                2001,
                x,
                y,
                ...
                if (repeatedUses == 0 || world.rand.nextInt(100) <= getLuckPercent()) {
                    if (fakeItemDye.onItemUse(fakeItemStack, player, world, x, y, z, side, xOff, yOff, zOff)) {
                        ...
                        player.worldObj.playSoundAtEntity(
                            ...
                            "random.orb",
                            0.1F,
                            0.5F * ((player.worldObj.rand.nextFloat() - player.worldObj.rand.nextFloat()) * 0.7F
                                + 1.2F));
                    }
                }
            }
