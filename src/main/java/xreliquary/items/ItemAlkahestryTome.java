...
public class ItemAlkahestryTome extends ItemToggleable {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (player.isSneaking()) return newStack;
    ...
    public void onUpdate(ItemStack ist, World world, Entity entity, int i, boolean f) {
        ...
        // redstone handler
        if (NBTHelper.getInteger("redstone", ist) + 9 <= getRedstoneLimit()
            && InventoryHelper.consumeItem(Blocks.redstone_block, player)) {
            NBTHelper.setInteger("redstone", ist, NBTHelper.getInteger("redstone", ist) + 9);
        } else if (NBTHelper.getInteger("redstone", ist) + 1 <= getRedstoneLimit()
            && InventoryHelper.consumeItem(Items.redstone, player)) {
                NBTHelper.setInteger("redstone", ist, NBTHelper.getInteger("redstone", ist) + 1);
            }

        // glowstone handler
        if (NBTHelper.getInteger("redstone", ist) + 4 <= getRedstoneLimit()
            && InventoryHelper.consumeItem(Blocks.glowstone, player)) {
            NBTHelper.setInteger("redstone", ist, NBTHelper.getInteger("redstone", ist) + 4);
        } else if (NBTHelper.getInteger("redstone", ist) + 1 <= getRedstoneLimit()
            && InventoryHelper.consumeItem(Items.glowstone_dust, player)) {
                NBTHelper.setInteger("redstone", ist, NBTHelper.getInteger("redstone", ist) + 1);
            }
    }
    ...
    public void addInformation(ItemStack ist, EntityPlayer par2EntityPlayer, List list, boolean par4) {
        ...
        this.formatTooltip(
            ImmutableMap.of(
                "redstoneAmount",
                String.valueOf(NBTHelper.getInteger("redstone", ist)),
                "redstoneLimit",
                String.valueOf(getRedstoneLimit())),
            ist,
            list);
        ...
        if (this.isEnabled(ist)) LanguageHelper.formatTooltip(
            ...
            ImmutableMap.of(
                "item",
                EnumChatFormatting.RED + Items.redstone.getItemStackDisplayName(new ItemStack(Items.redstone))),
            ist,
            list);
