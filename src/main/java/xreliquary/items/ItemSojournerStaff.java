...
public class ItemSojournerStaff extends ItemToggleable {
    ...
    private void scanForMatchingTorchesToFillInternalStorage(ItemStack ist, EntityPlayer player) {
        ...
        for (String torch : torches) {
            if (!items.contains(Reliquary.CONTENT.getItem(torch))) items.add(Reliquary.CONTENT.getItem(torch));
        }
        ...
        if (torchToPlace != null) {
            for (int i = 0; i < tagList.tagCount(); ++i) {
                ...
                if (itemName.equals(torchToPlace)) {
                    int quantity = tagItemData.getInteger("Quantity");
                    if (quantity <= 0) torchToPlace = null;
                }
            }
        }
    ...
    private void cycleTorchMode(ItemStack ist) {
        ...
        for (int i = 0; i < tagList.tagCount(); ++i) {
            ...
            if (itemName.equals(mode)) itemFound = true;
    ...
    public void addInformation(ItemStack ist, EntityPlayer player, List list, boolean par4) {
        ...
        if (tagCompound != null) {
            ...
            for (int i = 0; i < tagList.tagCount(); ++i) {
                ...
                phrase = String.format(
                    "%s%s",
                    phrase.equals("Nothing.") ? "" : String.format("%s;", phrase),
                    new ItemStack(containedItem, 1, 0).getDisplayName() + ": " + quantity);
            }
        ...
        if (this.isEnabled(ist)) LanguageHelper.formatTooltip(
            ...
            ImmutableMap.of("item", EnumChatFormatting.YELLOW + getItemStackDisplayName(new ItemStack(Blocks.torch))),
            ist,
            list);
    ...
    @Override
    public boolean onItemUse(ItemStack ist, EntityPlayer player, World world, int x, int y, int z, int side, float xOff,
        ...
        if (getTorchPlacementMode(ist) == null) return false;
        Block blockAttemptingPlacement = Block.getBlockFromName(getTorchPlacementMode(ist));
        if (blockAttemptingPlacement == null) return false;

        Block blockTargetted = world.getBlock(x, y, z);

        if (ContentHelper.areBlocksEqual(blockTargetted, Blocks.snow)) {
            side = 1;
        } else if (!ContentHelper.areBlocksEqual(blockTargetted, Blocks.vine)
            && !ContentHelper.areBlocksEqual(blockTargetted, Blocks.tallgrass)
            && !ContentHelper.areBlocksEqual(blockTargetted, Blocks.deadbush)
            && (blockTargetted == null || !blockTargetted.isReplaceable(world, x, y, z))) {
        ...
        if (blockAttemptingPlacement.canPlaceBlockAt(world, x, y, z)) {
            if (world.canPlaceEntityOnSide(blockAttemptingPlacement, x, y, z, false, side, player, ist)) {
                if (!player.capabilities.isCreativeMode) {
                    ...
                    for (; distance > Reliquary.CONFIG
                        .getInt(Names.sojourner_staff, "tile_per_cost_multiplier"); distance -= Reliquary.CONFIG
                            .getInt(Names.sojourner_staff, "tile_per_cost_multiplier")) {
                        cost++;
                    }
                ...
                if (placeBlockAt(
                    ...
                    yOff,
                    zOff,
                    attemptSide(world, x, y, z, side, blockAttemptingPlacement),
                    blockAttemptingPlacement)) {
                    ...
                    world.playSoundEffect(
                        ...
                        y + 0.5F,
                        z + 0.5F,
                        blockAttemptingPlacement.stepSound.getStepResourcePath(),
                        (blockAttemptingPlacement.stepSound.getVolume() + 1.0F) / 2.0F,
                        blockAttemptingPlacement.stepSound.getPitch() * 0.8F);
                }
            }
        }
        return true;
    ...
    public ItemStack onItemRightClick(ItemStack ist, World world, EntityPlayer player) {
        // calls onItemUse so all of the functionality we'd normally have to do preventative checks on gets handled
        // there.
    ...
    // I named the vars in this method weird crap cos I have no idea what they do. This was stolen from the bucket code,
    // I think.
    @Override
    protected MovingObjectPosition getMovingObjectPositionFromPlayer(World world, EntityPlayer player,
        boolean weirdBucketBoolean) {
        float movementCoefficient = 1.0F;
        float pitchOff = player.prevRotationPitch
            + (player.rotationPitch - player.prevRotationPitch) * movementCoefficient;
        ...
        Vec3 rayTraceVector = playerVector
            .addVector(pythagoraStuff * distCoeff, sinTracePitch * distCoeff, pythagoraStuff2 * distCoeff);
