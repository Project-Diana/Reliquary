...
public class ItemEnderStaff extends ItemToggleable {
    ...
    private int getEnderPearlLimit() {
        return Reliquary.CONFIG.getInt(Names.ender_staff, "ender_pearl_limit");
    }
    ...
    public float getDigSpeed(ItemStack ist, Block block, int meta) {
        // temporarily sets the item damage to 1, this prevents it from being rapid fired due to swing animations during
        // an attempt to break blocks, hopefully.
        if (ist.getItemDamage() == 0) ist.setItemDamage(1);
        return 0F;
    }
    ...
    public void cycleMode(ItemStack ist) {
        if (getMode(ist).equals("cast")) setMode(ist, "long_cast");
        else if (getMode(ist).equals("long_cast")) setMode(ist, "node_warp");
        else setMode(ist, "cast");
    }
    ...
    public void onUpdate(ItemStack ist, World world, Entity e, int i, boolean b) {
        ...
        if (NBTHelper.getInteger("ender_pearls", ist) + getEnderPearlWorth() <= getEnderPearlLimit()) {
            if (InventoryHelper.consumeItem(new ItemStack(Items.ender_pearl), player)) {
                NBTHelper
                    .setInteger("ender_pearls", ist, NBTHelper.getInteger("ender_pearls", ist) + getEnderPearlWorth());
            }
        }
    }
    ...
    public ItemStack onItemRightClick(ItemStack ist, World world, EntityPlayer player) {
        if (!player.isSneaking()) {
            if (getMode(ist).equals("cast") || getMode(ist).equals("long_cast")) {
                ...
                player.swingItem();
                if (NBTHelper.getInteger("ender_pearls", ist) < getEnderStaffPearlCost()) return ist;
                player.worldObj
                    .playSoundAtEntity(player, "random.bow", 0.5F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
                if (!player.worldObj.isRemote) {
                    player.worldObj.spawnEntityInWorld(
                        new EntityEnderStaffProjectile(player.worldObj, player, !getMode(ist).equals("long_cast")));
                    NBTHelper.setInteger(
                        "ender_pearls",
                        ist,
                        NBTHelper.getInteger("ender_pearls", ist) - getEnderStaffPearlCost());
                }
                // setCooldown(ist);
            } else {
    ...
    private ItemStack doWraithNodeWarpCheck(ItemStack ist, World world, EntityPlayer player) {
        ...
        // return ist;
        if (NBTHelper.getInteger("ender_pearls", ist) < getEnderStaffNodeWarpCost()) return ist;

        if (ist.getTagCompound() != null && ist.getTagCompound()
            .getInteger("dimensionID") != Integer.valueOf(getWorld(player))) {
            if (!world.isRemote) {
                player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.DARK_RED + "Out of range!"));
            }
        } else if (ist.getTagCompound() != null && ContentHelper.areBlocksEqual(
            world.getBlock(
                ist.getTagCompound()
                    .getInteger("nodeX" + getWorld(player)),
                ist.getTagCompound()
                    .getInteger("nodeY" + getWorld(player)),
                ist.getTagCompound()
                    .getInteger("nodeZ" + getWorld(player))),
            Reliquary.CONTENT.getBlock(Names.wraith_node))) {
                if (canTeleport(
                    ...
                    NBTHelper.setInteger(
                        "ender_pearls",
                        ist,
                        NBTHelper.getInteger("ender_pearls", ist) - getEnderStaffNodeWarpCost());
                }
            } else if (ist.getTagCompound() != null && ist.getTagCompound()
                .hasKey("dimensionID")) {
                    ist.getTagCompound()
                        .removeTag("dimensionID");
                    ist.getTagCompound()
                        .removeTag("nodeX");
                    ist.getTagCompound()
                        .removeTag("nodeY");
                    ...
                    if (!world.isRemote) {
                        player.addChatComponentMessage(
                            new ChatComponentText(EnumChatFormatting.DARK_RED + "Node dosen't exist!"));
                    } else {
    ...
    public void addInformation(ItemStack ist, EntityPlayer player, List list, boolean flag) {
        ...
        if (ist.getTagCompound() != null && ist.getTagCompound()
            ...
            .hasKey("nodeX" + getWorld(player))
            && ist.getTagCompound()
                .hasKey("nodeY" + getWorld(player))
            && ist.getTagCompound()
                .hasKey("nodeZ" + getWorld(player))) {
                    position = "X: " + ist.getTagCompound()
                        .getInteger("nodeX" + getWorld(player))
                        + " Y: "
                        + ist.getTagCompound()
                            .getInteger("nodeY" + getWorld(player))
        ...
        if (this.isEnabled(ist)) LanguageHelper.formatTooltip(
            ...
            ImmutableMap.of(
                "item",
                EnumChatFormatting.GREEN + Items.ender_pearl.getItemStackDisplayName(new ItemStack(Items.ender_pearl))),
            ist,
            list);
    ...
    public boolean onItemUse(ItemStack ist, EntityPlayer player, World world, int x, int y, int z, int side, float xOff,
        ...
        if ((ist.getTagCompound() == null || !(ist.getTagCompound()
            .hasKey("dimensionID")))
            && ContentHelper.areBlocksEqual(world.getBlock(x, y, z), Reliquary.CONTENT.getBlock(Names.wraith_node))) {
    ...
    public String getWorld(EntityPlayer player) {
        return Integer.valueOf(player.worldObj.provider.dimensionId)
            .toString();
    }
}
