...
public class ItemShearsOfWinter extends ItemBase {
    ...
    @Override
    public boolean onBlockDestroyed(ItemStack ist, World world, Block block, int x, int y, int z,
    ...
    public float func_150893_a(ItemStack ist, Block block) {
        return block != Blocks.web && block.getMaterial() != Material.leaves
            ? (block == Blocks.wool ? 5.0F : super.func_150893_a(ist, block))
            : 15.0F;
    }
    ...
    public void onUsingTick(ItemStack ist, EntityPlayer player, int count) {
        ...
        if (getMaxItemUseDuration(ist) - count <= 5) return;
        Vec3 lookVector = player.getLookVec();
        spawnBlizzardParticles(lookVector, player);

        doEntityShearableCheck(ist, player, lookVector);
        if (lookVector.xCoord > 0) doPositiveXCheck(ist, player, lookVector);
        else doNegativeXCheck(ist, player, lookVector);
    ...
    public void doPositiveXCheck(ItemStack ist, EntityPlayer player, Vec3 lookVector) {
        ...
        for (int x = 0; x < (int) (lookVector.xCoord * 10D); x++) {
            firedOnce = true;
            if (lookVector.yCoord > 0) doPositiveYCheck(ist, player, lookVector, x);
            else doNegativeYCheck(ist, player, lookVector, x);
        }
    ...
    public void doPositiveYCheck(ItemStack ist, EntityPlayer player, Vec3 lookVector, int x) {
        ...
        for (int y = 0; y < (int) (lookVector.yCoord * 10D); y++) {
            firedOnce = true;
            if (lookVector.zCoord > 0) doPositiveZCheck(ist, player, lookVector, x, y);
            else doNegativeZCheck(ist, player, lookVector, x, y);
        }
    ...
    public void doPositiveZCheck(ItemStack ist, EntityPlayer player, Vec3 lookVector, int x, int y) {
        ...
        if (!firedOnce) {
            for (int z = -2; z <= 2; z++) checkAndBreakBlockAt(x, y, z, player, ist);
        }
    }
        ...
        if (player.worldObj.rand.nextInt(probabilityFactor) == 0) {
            ...
            if (block instanceof IShearable) {
                ...
                if (target.isShearable(new ItemStack(Items.shears, 1, 0), player.worldObj, x, y, z)) {
                    ...
                    // ArrayList<ItemStack> drops = target.onSheared(new ItemStack(Items.shears, 1, 0), player.worldObj,
                    // x, y, z,
                    // EnchantmentHelper.getEnchantmentLevel(Enchantment.fortune.effectId, ist));
                    ArrayList<ItemStack> drops = block.getDrops(
                        player.worldObj,
                        x,
                        y,
                        z,
                    ...
                    if (player.worldObj.isRemote) {
                        if (block.getMaterial() != Material.air) player.worldObj.playAuxSFXAtEntity(
                            player,
                            2001,
                            x,
                            y,
                            z,
                            Block.getIdFromBlock(block) + (player.worldObj.getBlockMetadata(x, y, z) << 12));
    ...
    public void doEntityShearableCheck(ItemStack ist, EntityPlayer player, Vec3 lookVector) {
        ...
        double lowerY = Math
            .min(player.posY + player.getEyeHeight(), player.posY + player.getEyeHeight() + lookVector.yCoord * 10D);
        ...
        double upperY = Math
            .max(player.posY + player.getEyeHeight(), player.posY + player.getEyeHeight() + lookVector.yCoord * 10D);
        ...
        while (iterator.hasNext()) {
            ...
            if (probabilityFactor > 0 && player.worldObj.rand.nextInt(probabilityFactor) != 0) continue;
    ...
    public void spawnBlizzardParticles(Vec3 lookVector, EntityPlayer player) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            // player.worldObj.spawnParticle("snowballpoof", player.posX + randX, player.posY + randY, player.posZ +
            // randZ, lookVector.xCoord * 5, lookVector.yCoord * 5, lookVector.zCoord * 5);
        }
