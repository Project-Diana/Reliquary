    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (var4 != null && var4.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            if (world.canMineBlock(player, var5, var6, var7)
                && player.canPlayerEdit(var5, var6, var7, var4.sideHit, stack)
                && world.getBlock(var5, var6, var7)
                    .getMaterial() == Material.water
                && world.getBlockMetadata(var5, var6, var7) == 0
                && world.isAirBlock(var5, var6 + 1, var7)) {
                world.setBlock(var5, var6 + 1, var7, Reliquary.CONTENT.getBlock(Names.lilypad));
                world.scheduleBlockUpdate(
                    var5,
                    var6 + 1,
                    var7,
                    Reliquary.CONTENT.getBlock(Names.lilypad),
                    (int) (1360F * ((float) getDelayInSeconds() / 100F)));
        ...
        if (!blockPlaced) {
            // on-block-click portion of the method, we're going to make this the "alternative" to trying to raytrace
            // first, since raytrace is optimal.
            ...
            if (stack.stackSize == 0) return false;
            ...
            else if (y == 255 && this.field_150939_a.getMaterial()
                ...
                // added a check to make sure that the block can exist at the location before actually placing the
                // thing.
                if (y >= 0 && y < 256
                    && world.getBlock(x, y - 1, z)
                        .getMaterial() == Material.water
                    && world.getBlockMetadata(x, y - 1, z) == 0
                    && placeBlockAt(stack, player, world, x, y, z, side, posX, posY, posZ, j1)) {
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World world, EntityPlayer par3EntityPlayer) {
        ...
        if (var4 == null) return par1ItemStack;
            ...
            if (var4.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                if (!world.canMineBlock(par3EntityPlayer, var5, var6, var7)) return par1ItemStack;

                if (!par3EntityPlayer.canPlayerEdit(var5, var6, var7, var4.sideHit, par1ItemStack))
                    return par1ItemStack;

                if (world.getBlock(var5, var6, var7)
                    .getMaterial() == Material.water && world.getBlockMetadata(var5, var6, var7) == 0
                    && world.isAirBlock(var5, var6 + 1, var7)) {
                    world.setBlock(var5, var6 + 1, var7, Reliquary.CONTENT.getBlock(Names.lilypad));
                    world.scheduleBlockUpdate(
                        var5,
                        ...
                        var7,
                        Reliquary.CONTENT.getBlock(Names.lilypad),
                        (int) (1200 * (float) getDelayInSeconds()));
