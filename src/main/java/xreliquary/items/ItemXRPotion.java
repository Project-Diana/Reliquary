...
public class ItemXRPotion extends ItemBase {
    ...
    public IIcon getIcon(ItemStack ist, int renderPass) {
        PotionEssence essence = new PotionEssence(ist.getTagCompound());
        boolean hasEffect = essence.getEffects()
            .size() > 0;
    ...
    public EnumAction getItemUseAction(ItemStack ist) {
        if (!getSplash(ist) && new PotionEssence(ist.getTagCompound()).getEffects()
            .size() > 0) return EnumAction.drink;
                ...
                if (mop == null) return ist;
                    ...
                    if (mop.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                        ...
                        if (world.getBlock(blockX, blockY, blockZ) instanceof BlockApothecaryCauldron) {
                            TileEntityCauldron cauldronTile = (TileEntityCauldron) world
