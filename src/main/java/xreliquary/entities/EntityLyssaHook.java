...
public class EntityLyssaHook extends EntityFishHook {
    ...
    private static final List garbageList = Arrays.asList(
    ...
    private static final List plantList = Arrays.asList(
    ...
    private static final List fishList = Arrays.asList(
    ...
    public EntityLyssaHook(World world, EntityPlayer player) {
        ...
        this.setLocationAndAngles(
            player.posX,
            player.posY + 1.62D - (double) player.yOffset,
            player.posZ,
            player.rotationYaw,
    ...
    public void onUpdate() {
        ...
        List pullingItemsList = this.worldObj.getEntitiesWithinAABB(
            EntityItem.class,
        ...
        while (itemIterator.hasNext()) {
            ...
            // itemEntity.setDead();
            // } else {
            Vec3 pullVector = Vec3.createVectorHelper(this.posX - e.posX, this.posY - e.posY, this.posZ - e.posZ)
                .normalize();
        ...
        if (this.field_146055_aB > 0) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (this.field_146042_b.isDead || !this.field_146042_b.isEntityAlive()
                    || itemstack == null
                    || itemstack.getItem() != Reliquary.CONTENT.getItem(Names.rod_of_lyssa)
                    || this.getDistanceSqToEntity(this.field_146042_b) > 4096.0D) {
                    this.setDead();
                ...
                if (this.inEntity != null) {
                    if (!this.inEntity.isDead) {
                        ...
                        this.posY = this.inEntity.boundingBox.minY + (double) this.inEntity.height * 0.8D;
            ...
            if (movingobjectposition != null) {
                if (movingobjectposition.entityHit != null) {
                    ...
                    List eList = worldObj.getEntitiesWithinAABB(
                        EntityLiving.class,
                        AxisAlignedBB.getBoundingBox(lowerX, lowerY, lowerZ, upperX, upperY, upperZ));
                    Iterator iterator = eList.iterator();
                    while (iterator.hasNext()) {
                        Entity e = (Entity) iterator.next();
                        if (e instanceof EntityLivingBase && !e.isEntityEqual(field_146042_b)) {
                            if (field_146042_b.isSneaking() || movingobjectposition.entityHit
                                .attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase) e), 0.0F)) {
            ...
            if (!this.field_146051_au) {
                ...
                for (int j = 0; j < b0; ++j) {
                    double d3 = this.boundingBox.minY
                        + (this.boundingBox.maxY - this.boundingBox.minY) * (double) (j + 0) / (double) b0
                        - 0.125D
                        + 0.125D;
                    double d4 = this.boundingBox.minY
                        + (this.boundingBox.maxY - this.boundingBox.minY) * (double) (j + 1) / (double) b0
                        - 0.125D
                        + 0.125D;
                    ...
                    if (this.field_146045_ax > 0) {
                        ...
                        if (this.field_146038_az > 0) {
                            ...
                            if (this.field_146038_az <= 0) {
                                ...
                                float f4 = f2 * 0.04F;
                                worldserver
                                    .func_147487_a("wake", d11, d5, d6, 0, (double) f4, 0.01D, (double) (-f3), 1.0D);
                                worldserver
                                    .func_147487_a("wake", d11, d5, d6, 0, (double) (-f4), 0.01D, (double) f3, 1.0D);
                            }
            ...
            if (this.inEntity != null) {
                ...
                if (field_146042_b.isSneaking() && this.inEntity instanceof EntityLivingBase) {
                    ...
                    if (!(livingBase instanceof EntityPlayer)) {
                        ...
                        if (stolenStack == null
                            && !Reliquary.CONFIG.getBool(Names.rod_of_lyssa, "steal_from_vacant_slots")) {
                            for (int i = 0; i < 5; ++i) {
                                stolenStack = livingBase.getEquipmentInSlot(i);
                                if (stolenStack != null) break;
                            }
                        }
                        ...
                        if (Reliquary.CONFIG.getBool(Names.rod_of_lyssa, "use_leveled_failure_rate"))
                            failProbabilityFactor = 1F / ((float) Math.sqrt(
                                (double) Math.max(
                                    1,
                                    Math.min(
                                        field_146042_b.experienceLevel,
                                        Reliquary.CONFIG.getInt(Names.rod_of_lyssa, "level_cap_for_leveled_formula"))))
                                * 2);
                        else failProbabilityFactor = (float) Reliquary.CONFIG
                            .getInt(Names.rod_of_lyssa, "flat_steal_failure_percent_rate") / 100F;

                        if (rand.nextFloat() <= failProbabilityFactor || (stolenStack == null
                            && Reliquary.CONFIG.getBool(Names.rod_of_lyssa, "fail_steal_from_vacant_slots"))) {
    ...
    private ItemStack func_146033_f() {
        ...
        if (f < f1) {
            ...
            return ((WeightedRandomFishable) WeightedRandom.getRandomItem(this.rand, garbageList))
            ...
            if (f < f2) {
                ...
                return ((WeightedRandomFishable) WeightedRandom.getRandomItem(this.rand, plantList))
                    .func_150708_a(this.rand);
            } else {
                float f3 = f - f2;
                this.field_146042_b.addStat(StatList.fishCaughtStat, 1);
                return ((WeightedRandomFishable) WeightedRandom.getRandomItem(this.rand, fishList))
                    .func_150708_a(this.rand);
            }
        }
    }
