 ...
 * Handles all the defaults for potion ingredients. As it stands there's a lot, so this is a pretty ugly class. Would be
 * nice to get it cleaned up but
...
public class PotionMap {
    ...
    public static void init() {
        ...
        // TIER ONE INGREDIENTS, these are always 0 potency and have minimal durations (3 for positive, 1 for negative
        // or super-positive)
        ...
        // TIER TWO INGREDIENTS, one of the effects of each will always be a one, slightly increased duration vs. TIER
        // ONE
        ...
        // TIER THREE INGREDIENTS, these are closer to vanilla durations, carry many effects or a slightly increased
        // duration. Some/most are combos.
        addPotionConfig(
            Items.pumpkin_pie,
            invis(1, 0),
            fireres(1, 0),
            ...
            haste(3, 0),
            absorb(3, 0),
            regen(0, 0)); // combination of ingredients, strong.
        ...
        // TIER FOUR INGREDIENTS, these carry multiple one-potency effects and have the most duration for any given
        // effect.
    ...
    private static void addPotionConfig(ItemStack ist, String... params) {
        ...
        configList.put(
            ContentHelper.getIdent(ist.getItem())
                .replace(':', '_') + (ist.getItemDamage() == 0 ? "" : "_" + ist.getItemDamage()),
            ingredientEffects);
    }

    public static void initializePotionMappings() {
        // _ replaces : in identity names. the last _ should always denote a meta value that separates this from another
        // item, if possible.

        Set<Map.Entry<String, Object>> configIngredients = Reliquary.CONFIG.getGroup(Names.potion_ingredient)
            .entrySet();
        for (Map.Entry<String, Object> configIngredient : configIngredients) {
            ...
            // we split the string a second time, but this time we're only doing this to match the first underscore so
            // we can replace it with a colon.
            // the string is limited to being split "in two", one for the mod_id and the other the actual item name, and
            // whatever meta it has having been dropped.
