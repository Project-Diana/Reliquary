...
public class BlockAlkahestryAltar extends BlockContainer {
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xOff,
        ...
        if (player.getCurrentEquippedItem()
            ...
            world.playSoundEffect(
                ...
                "random.fizz",
                0.3F,
                0.5F + 0.5F * altar.getRedstoneCount() + (float) (world.rand.nextGaussian() / 8));
            for (int particles = world.rand.nextInt(3); particles
                < 3 + altar.getRedstoneCount() * 4 + altar.getRedstoneCount(); particles++) {
                world.spawnParticle(
                    "reddust",
                    x + 0.5D + world.rand.nextGaussian() / 5,
                    y + 1.2D,
                    z + 0.5D + world.rand.nextGaussian() / 5,
                    1D,
                    0D,
                    0D);
            }
            ...
            && NBTHelper.getInteger("redstone", player.getCurrentEquippedItem()) > 0) {
                world.playSoundEffect(
                    x + 0.5F,
                    y + 0.5F,
                    z + 0.5F,
                ...
                NBTHelper.setInteger(
                    ...
                    NBTHelper.getInteger("redstone", player.getCurrentEquippedItem()) - 1);
                altar.addRedstone();
            }
        return true;
    }
    ...
    private int getSlotWithRedstoneDust(EntityPlayer player) {
        for (int slot = 0; slot < player.inventory.mainInventory.length; slot++) {
            ...
            if (player.inventory.mainInventory[slot].getItem() == Items.redstone) return slot;
        }
        return -1;
    }
