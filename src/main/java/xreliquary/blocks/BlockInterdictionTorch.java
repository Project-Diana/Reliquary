...
public class BlockInterdictionTorch extends BlockTorch implements ICustomItemBlock {
    ...
    public void updateTick(World world, int x, int y, int z, Random random) {
        ...
        List<String> entitiesThatCanBePushed = (List<String>) Reliquary.CONFIG
            .get(Names.interdiction_torch, "entities_that_can_be_pushed");
        List<String> projectilesThatCanBePushed = (List<String>) Reliquary.CONFIG
            .get(Names.interdiction_torch, "projectiles_that_can_be_pushed");
        ...
        for (Entity entity : entities) {
            ...
            if (entitiesThatCanBePushed.contains(entityName) || (projectilesThatCanBePushed.contains(entityName)
                && Reliquary.CONFIG.getBool(Names.interdiction_torch, "can_push_projectiles"))) {
                ...
                Vec3 angleOfAttack = Vec3
                    .createVectorHelper(entity.posX - (x + 0.5D), entity.posY - y, entity.posZ - (z + 0.5D));
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random) {
        ...
        if (orientation == 1) {
            world.spawnParticle(
                "mobSpell",
                xOffset - horizontalModifier,
                yOffset + verticalModifier,
                zOffset,
                0.0D,
                0.0D,
                0.0D);
