...
public class TileEntityCauldron extends TileEntityBase {
    ...
    public void updateEntity() {
        ...
        if (getHeatSources().contains(worldObj.getBlock(xCoord, yCoord - 1, zCoord))
            && worldObj.getBlockMetadata(xCoord, yCoord, zCoord) > 0) {
            if (potionEssence != null) {
                if (cookTime < getCookTime()) cookTime++;
            }
            if (worldObj.isRemote) {
                for (int particleCount = 0; particleCount <= 2; ++particleCount) spawnBoilingParticles();
    ...
    public void spawnBoilingParticles() {
        if (worldObj.rand.nextInt(getCookTime() * getCookTime()) > cookTime * cookTime) return;
        ...
        EntityCauldronBubbleFX bubble = new EntityCauldronBubbleFX(
            worldObj,
            xCoord + 0.5D + xOffset,
            yCoord + 0.01D + BlockCauldron.getRenderLiquidLevel(worldObj.getBlockMetadata(xCoord, yCoord, zCoord)),
            zCoord + 0.5D + zOffset,
            0D,
            0D,
            0D,
            ...
            green,
            blue);
        EntityCauldronSteamFX steam = new EntityCauldronSteamFX(
            worldObj,
            xCoord + 0.5D + xOffset,
            yCoord + 0.01D + BlockCauldron.getRenderLiquidLevel(worldObj.getBlockMetadata(xCoord, yCoord, zCoord)),
            zCoord + 0.5D + zOffset,
            0D,
            0.05D + 0.02F * BlockCauldron.getRenderLiquidLevel(worldObj.getBlockMetadata(xCoord, yCoord, zCoord)),
            0D,
            red,
            green,
            blue);
        FMLClientHandler.instance()
            .getClient().effectRenderer.addEffect(bubble);
        if (worldObj.rand.nextInt(6) == 0) FMLClientHandler.instance()
            .getClient().effectRenderer.addEffect(steam);
    }
    ...
    public void spawnGunpowderParticles() {
        if (worldObj.rand.nextInt(8) > 0) return;
        float xOffset = (worldObj.rand.nextFloat() - 0.5F) / 1.66F;
        float zOffset = (worldObj.rand.nextFloat() - 0.5F) / 1.66F;
        worldObj.spawnParticle(
            "smoke",
            xCoord + 0.5D + xOffset,
            yCoord + BlockCauldron.getRenderLiquidLevel(worldObj.getBlockMetadata(xCoord, yCoord, zCoord)),
            zCoord + 0.5D + zOffset,
            0.0D,
            0.1D,
            0.0D);
    ...
    public void spawnRedstoneParticles() {
        if (worldObj.rand.nextInt(10) / this.redstoneCount > 0) return;
        float xOffset = (worldObj.rand.nextFloat() - 0.5F) / 1.66F;
        float zOffset = (worldObj.rand.nextFloat() - 0.5F) / 1.66F;
        worldObj.spawnParticle(
            "reddust",
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        if (potionEssence.getEffects()
            .size() == 0) this.potionEssence = null;
    }
    ...
    public boolean isItemValidForInput(ItemStack ist) {
        if (worldObj.getBlockMetadata(xCoord, yCoord, zCoord) < 3) return false;
    ...
    public List<Block> getHeatSources() {
        ...
        List<String> heatSourceBlockNames = (List<String>) Reliquary.CONFIG
            .get(Names.apothecary_cauldron, "heat_sources");
