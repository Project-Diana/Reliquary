...
public class TileEntityMortar extends TileEntityInventory {
    ...
    public void usePestle() {
        ...
        if (pestleUsedCounter >= Reference.PESTLE_USAGE_MAX) {
            ...
            PotionEssence resultEssence = new PotionEssence(
                potionIngredients.toArray(new PotionIngredient[potionIngredients.size()]));
            if (resultEssence.getEffects()
                .size() == 0) {
                pestleUsedCounter = 0;
                for (int clearSlot = 0; clearSlot < this.getSizeInventory(); ++clearSlot) {
                    if (this.getStackInSlot(clearSlot) == null) continue;
