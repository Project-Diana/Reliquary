 ...
 * <p/>
 * Put the @ContentInit annotation on any block/item class (or subclass) within the blocksPath/itemsPath you assigned
 * for your @SandstoneMod, and Content will take care of the rest.
...
public class Content {
     ...
     *
     * @param classLoader The ClassLoader from any class inside the mod. Most reliably, use the class with the @Mod
     *                    annotation. This must come from a class within the mod's jar file.
            ...
            if (obj instanceof Item) {
                ...
                if (obj instanceof ICustomItemBlock) {
                    GameRegistry.registerBlock(
                        block,
                        ((ICustomItemBlock) obj).getCustomItemBlock(),
                ...
                Sandstone.LOGGER.warn(
                    "Class '" + objClass.getName()
                        + "' is not a Block or an Item! You shouldn't be calling @ContentInit on this! Ignoring!");
            }
        }
    }
    ...
    public Block getBlock(String blockName) {
        ...
        if (!selection.contains(":")) selection = ModRegistry.getID(modId) + ":" + selection;
        if (selection.indexOf(":") == 0) selection = selection.substring(1);
